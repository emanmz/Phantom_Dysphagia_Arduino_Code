...
// Horizontal actuator (13.04 mm stroke) - Assuming this is the 'Z' axis for calibration
#define dirPinH 2
#define stepPinH 3
const float steps_per_mm_horizontal = 613.50;
const float max_position_mm_horizontal = 25.00;
const float max_position_mm_z = max_position_mm_horizontal; 
...
float cal_slope_z = 0; 
float cal_offset_z = -0.776723;
//float cal_slope_z = 1; 
//float cal_offset_z = 0;
...
    Serial.println("Are you ready for Z calibration? Make sure the initial movement before calibration is set in function (Type 'Ready' to begin)");
...
    Serial.println("Are you ready for Y calibration? Make sure the initial movement before calibration is set in function(Type 'Ready' to begin)");
    // Wait for the next input, which should be 'Ready'
    while (!Serial.available());
    String readyCheck = Serial.readStringUntil('\n');
    readyCheck.trim();
...
    moveBoth(0, horizSpeed, 0, vertSpeed); // set back to 0 0 
  }
}
...
  Serial.println("Starting Z calibration...");

  // Calibrate over the entire horizontal stroke (13.04 mm) with 0.1 mm steps
  float stepSize = 0.1;
  // *** FIX 3: Use an int cast for the result of round for array size calculation ***
  const int N_MAX = (int)round(max_position_mm_z / stepSize) + 1; 
  float mm_commanded_List[N_MAX]; // z-data (Commanded Actuator mm)
...
  for (float mm_cmd = 0.0; /*valid_count < 15;*/ mm_cmd += stepSize) { // Removed 'valid_count < 15' constraint to run full stroke
    //Serial.println(idx);
    //Serial.println(N_MAX);
    // Stop if commanded position exceeds max stroke
    if (mm_cmd > max_position_mm_z){
      Serial.print("Reached max Z position.");
      break;
    }
...
      // Check for array bounds before writing
      if (valid_count < N_MAX) {
...
        valid_count++;
      } else {
        // Should not happen if N_MAX calculation is correct, but safer to check
        Serial.println("ERROR: Array overflow during Z calibration.");
        break; 
      }
    } else {
...
  } else {
    linearRegression(mm_commanded_List, mm_measured_List, valid_count, cal_slope_z, cal_offset_z);
...
    Serial.println(cal_slope_z, 6);
    Serial.print("New Cal Offset (mm): ");
    Serial.println(cal_offset_z, 6);
  
  }
  
...
// =============================================================
void calibrate_y() {
  Serial.println("Moving Z axis for LVDT to sit flush with calibration block...");
  float z_coord_for_calib = 0.0;
...
  float stepSize = 0.1;
  const int N_MAX = (int)round(CAL_Y_MAX_MM / stepSize) + 1; 
...
    moveBoth_Without_Calib(z_coord_for_calib, 0, mm_cmd, 10);
    delay(300);
...
        valid_count++;
      } else {
        Serial.println("ERROR: Array overflow during Y calibration.");
        break;
      }
    } else {
      Serial.print("IGNORED Data - Cmd: ");
...
  // Note: It seems the original intent of the calibration was to solve for the commanded_mm 
  // without the offset (as per the commented-out line in linearRegression), 
  // but the existing code uses the offset variable 'cal_offset_z' and 'cal_offset_Y'. 
  // Using the full formula:
  float horizMM = (realHorizMM - cal_offset_z) / cal_slope_z;
...
  int delayH = max(80, (int)round(1000000.0 / (horizSpeed_mmps * steps_per_mm_horizontal)));
  int delayV = max(80, (int)round(1000000.0 / (vertSpeed_mmps * steps_per_mm_vertical)));
...
  int delayH = (int)round(1000000.0 / (horizSpeed_mmps * steps_per_mm_horizontal));
  int delayV = (int)round(1000000.0 / (vertSpeed_mmps * steps_per_mm_vertical));
